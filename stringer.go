...
var (
	nosql           = flag.Bool("nosql", false, "if true, the Scanner and Valuer interfaces will not be implemented")
	nojson          = flag.Bool("nojson", false, "if true, json marshaling methods will not be generated")
	yaml            = flag.Bool("yaml", false, "if true, yaml marshaling methods will be generated")
	text            = flag.Bool("text", false, "if true, text marshaling methods will be generated")
	output          = flag.String("output", "plenum.gen.go", "output file name")
	transformMethod = flag.String("transform", "noop", "enum item name transformation method")
	trimPrefix      = flag.String("trimprefix", "", "transform each item name by removing a prefix (default none)")
)

// Usage is a replacement usage function for the flags package.
func Usage() {
	fmt.Fprintf(os.Stderr, "Usage of %s:\n", os.Args[0])
	fmt.Fprintf(os.Stderr, "\t//go:generate goplenum Type [flags]\n")
	fmt.Fprintf(os.Stderr, "For more information, see:\n")
	fmt.Fprintf(os.Stderr, "\thttp://github.com/tucnak/goplenum\n\n")
...
func main() {
	...
	// Run generate
	g.generate(typeName, !*nojson, *yaml, !*nosql, *text, *transformMethod, *trimPrefix)
	...
	if err := writeTypeToFile(g.pkg.name, outputName, typeName, src); err != nil {
		log.Fatalf("cannot write to file: %v", err)
	}
}

func writeTypeToFile(pkgName, outputName, typeName string, src []byte) error {
	begin, end := []byte("\n//BEGIN("+typeName+")"), []byte("\n//END("+typeName+")\n")
	...
	if _, err := os.Stat(outputName); err == nil {
		...
		start, fin := bytes.Index(content, begin)+len(begin), bytes.Index(content, end)
		...
		if start == -1 || fin == -1 {
			...
			// append to file
			f, err := os.OpenFile(outputName, os.O_RDWR|os.O_APPEND, 0660)
			if err != nil {
				return err
			}
			defer f.Close()
		...
		return ioutil.WriteFile(outputName, b.Bytes(), 0644)
	}

	b := &bytes.Buffer{}

	// Print the header and package clause.
	fmt.Fprintf(b, "// Code generated by package github.com/tucnak/goplenum\n")
	fmt.Fprintf(b, "package %s", pkgName)
	fmt.Fprintf(b, "\n\n")
	fmt.Fprintf(b, "import (\n")
	if !*nosql {
		fmt.Fprintf(b, "\t\"database/sql/driver\"\n")
	}
	if !*nojson {
...
func (g *Generator) createIndexAndNameDecl(run []Value, typeName string, suffix string) (string, string) {
	...
	nameConst := fmt.Sprintf("plenum%sRepr%s = %q", typeName, suffix, b.String())
	nameLen := b.Len()
	b.Reset()
	fmt.Fprintf(b, "plenum%sIndex%s = [...]uint%d{0, ", typeName, suffix, usize(nameLen))
...
func (g *Generator) declareNameVars(runs [][]Value, typeName string, suffix string) {
	g.Printf("const plenum%sRepr%s = \"", typeName, suffix)
...
func (i %[1]s) String() string {
	if %[3]si >= %[1]s(len(plenum%[1]sIndex)-1) {
		return fmt.Sprintf("%[1]s(%%d)", i)
	}
	return plenum%[1]sRepr[plenum%[1]sIndex[i]:plenum%[1]sIndex[i+1]]
}
`
...
func (i %[1]s) String() string {
	i -= %[2]s
	if %[4]si >= %[1]s(len(plenum%[1]sIndex)-1) {
...
func (g *Generator) buildMultipleRuns(runs [][]Value, typeName string) {
	...
	for i, values := range runs {
		if len(values) == 1 {
			g.Printf("\tcase i == %s:\n", &values[0])
			g.Printf("\t\treturn plenum%sRepr%d\n", typeName, i)
			continue
		}
		...
		g.Printf("\t\treturn plenum%sRepr%d[plenum%sIndex%d[i]:plenum%sIndex%d[i+1]]\n",
...
func (g *Generator) buildMap(runs [][]Value, typeName string) {
	...
	g.Printf("\nvar plenum%sMap = map[%s]string{\n", typeName, typeName)
	n := 0
	for _, values := range runs {
		for _, value := range values {
			g.Printf("\t%s: plenum%sRepr[%d:%d],\n", &value, typeName, n, n+len(value.name))
			n += len(value.name)
		}
	}
	g.Printf("}\n\n")
